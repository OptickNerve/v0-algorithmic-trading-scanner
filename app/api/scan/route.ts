  ...
  await Promise.all(
    symbols.map(async (symbol) => {
      ...
      try {
        ...
        // --- 🛑 FAIL-SAFE WEEKEND FALLBACK ---
        console.log(`Live API failed for ${symbol}. Injecting safety fallback data.`);
        
        let fallbackPrice = 1.08540 // Default Forex layout
        ...
        results.push({
          ...
          signal: 'NEUTRAL', // Keeps scanner quiet when data APIs are frozen
          confidence: 0,
